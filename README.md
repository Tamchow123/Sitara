...
Deliberately **not yet** implemented (later phases): email verification and password recovery, the single-refinement limit enforcement, retention/deletion, quotas and cost ledgers. Signed design-image delivery arrived in Phase 11 as short-lived bearer URLs (ownership checked before issuance; no revocation before expiry — a backend proxy is the documented upgrade path).
