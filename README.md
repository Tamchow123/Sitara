...
```
DEFAULT_IMAGE_MODEL = black-forest-labs/flux-2-max     (ADR 0019; accepts reference images)
...
`MAX_DESIGN_VERSIONS` (default 4 = initial concept + `MAX_REFINEMENTS`) caps
...
Deliberately **not yet** implemented (later phases): email verification and password recovery, retention/deletion, quotas and cost ledgers. The refinement limit arrived in Phase 14 (`DesignVersion.parent_version`/`refined_versions`) and is now `MAX_REFINEMENTS = 3` (ADR 0029). Signed design-image delivery arrived in Phase 11 as short-lived bearer URLs (ownership checked before issuance; no revocation before expiry — a backend proxy is the documented upgrade path).
