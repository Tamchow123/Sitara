...
**Up next — Phase 15: deterministic zero-cost demo engine.** Replaces the earlier fixture-lookup sketch: `DEMO_MODE=true` will build a real DesignSpec locally, reuse the existing deterministic image-prompt builder unchanged, deterministically select from a small reviewed fixture-image manifest, and run the result through the same durable pipeline, storage, job/result APIs and UI as live generation — never a separate toy frontend and never a mock hidden behind the paid-provider wrapper. See `docs/phases/PHASES.md`.
