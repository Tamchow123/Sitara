...
- **Scope:** revises the earlier fixture-lookup sketch into a deterministic, production-shaped local generation branch — never a second toy frontend and never a mock hidden behind the `ai_gateway` paid-provider wrapper. Pipeline: validated questionnaire → deterministic local DesignSpec builder → the existing deterministic image-prompt builder (unchanged `PROMPT_BUILDER_VERSION`) → a deterministic reviewed demo-fixture selector → local pre-generated WebP fixture bytes → the existing private staging and canonical image ingest → the existing job, progress, result and refinement UI.
  ...
  - *Deterministic DesignSpec generation:* a local builder assembles a complete, schema-valid DesignSpec from the canonical questionnaire selections, the linked questionnaire version, a safe controlled vocabulary, the Phase 13 inspiration-context snapshot where applicable, and a selected curated demo concept profile. Same canonical input + fixture-manifest version → the same result, `source_selections` preserved exactly, the authoritative DesignSpec schema and existing safety scan both run, no raw questionnaire free text interpolated into generated narrative, no LLM.
  - *Deterministic image selection:* a versioned, strict fixture manifest of roughly 30–50 curated concepts (target ~36) collectively covering every controlled garment type, the ceremony taxonomy, minimal/moderate/heavy embellishment, modest and full-coverage options, and varied fabric/palette/drape/silhouette combinations — without forcing every garment×ceremony cross-product where that would be culturally artificial. Selection is deterministic: exact garment type mandatory, exact ceremony strongly preferred, remaining controlled fields score against a documented weighted rubric, stable hashing breaks ties independent of manifest order; the selected fixture id and selector version are persisted on the DesignVersion for reproducibility.
  ...
  - *Deterministic refinement:* demo mode supports the complete Phase 14 refinement journey through a local refinement engine that accepts the same strict `RefinementRequest`, uses the same `change_type` allowlist and the existing exact DesignSpec diff validator, preserves `source_selections` and the historical inspiration context, creates version 2 only, and selects a deterministic alternate fixture (the optional free-text note may influence the variant choice through a hash, but raw note text is never copied into the DesignSpec or exposed publicly).
  - *Curated fixture pack:* demo images are project-generated/owned/commissioned, explicitly approved for public demo display, reviewed for cultural coherence, garment accuracy, coverage, and anatomy, free of logos/text/watermarks/designer imitation, sanitised single-frame WebP with all metadata stripped, bounded in dimensions/bytes, content-hashed, and referenced only through the strict manifest — never downloaded or fabricated. If the reviewed production pack is absent, incomplete, or corrupt, demo generation fails closed as unavailable rather than silently falling back to a generic placeholder; a separate manual, budget-authorised process creates the final pack, and automated tests use generated synthetic images only.
  - *Honest labelling:* the API uses the same contract for demo and live results with additive mode metadata in both paths; the UI identifies demo generation, representative pre-generated imagery, zero paid AI calls, possible mismatch between very specific answers and the nearest fixture, and concept-visualisation limitations — never implying a demo fixture was freshly produced by an AI provider.
  - *Showcase:* the same reviewed fixture manifest powers a read-only public showcase endpoint and landing-page carousel — no session created, no private design exposed, no storage path exposed — that Phase 16 may later use as its fallback when live generation is unavailable, throttled, or over budget.
- **Non-goals:** demo mode never imports or reaches the `ai_gateway` paid-provider wrappers (a distinct local branch in the generation layer, not a mocked wrapper); no rate-limited-paid anything labelled "demo"; no rate limits, cost ceilings, or retention (Phase 16 owns those — Phase 15 only supplies the showcase Phase 16 may fall back to); no automated import of untracked Phase 2 evaluation outputs into the fixture pack; no second, unrelated gallery dataset; no every-garment-×-every-ceremony cross-product where it would be culturally weak.
- **Commands:** `DEMO_MODE=true docker compose up -d`; `docker compose exec api pytest -k demo`; `docker compose exec web npm test -- --run`.
- **Automated tests:** with `DEMO_MODE=true`, a full generate + refine journey completes with **zero provider-wrapper invocations and no provider client construction (asserted via instrumentation)** and no network egress to provider hosts (socket-blocking test fixture); the same questionnaire input and manifest version always select the same fixture (determinism test); the local DesignSpec builder's output passes the authoritative schema and safety scan; response shapes validate against the OpenAPI schema identically to live mode; status transitions occur in order; an absent/incomplete/corrupt fixture pack fails closed rather than falling back to a placeholder.
- **Manual checkpoint:** run the entire user journey (questionnaire → generate → results → refine) in demo mode with no API keys configured at all; confirm it is indistinguishable in shape from live mode, honest in its demo labelling, and that the showcase carousel renders from the same reviewed manifest.
- **Commit:** `feat: deterministic zero-cost demo engine with reviewed fixture manifest and showcase`

## Phase 16 — Security and live-generation cost controls
- **Scope:** `LIVE_GENERATION_ENABLED` flag with `503 live_generation_disabled` when off; per-session + per-IP DRF/Redis throttles and daily generation count limit (`429 generation_limit_reached`); **hard daily cost ceiling with atomic reserve-before-spend**: compute a conservative maximum estimated cost for the call, atomically reserve it in Redis via a Lua script (or equivalent atomic transaction) *before* provider invocation, reject with `503 live_generation_budget_exhausted` if the reservation would exceed the ceiling, reconcile the reservation down to the estimated actual cost after the call, and release unused reservation on failure paths where no spend occurred — no check-then-increment race between workers; when live generation is disabled, throttled, or over budget, the frontend may degrade to the Phase 15 showcase gallery rather than a bare error. Security hardening pass (headers, CSP, cookie flags, admin lockdown; CORS surface stays minimal thanks to same-origin `/api/`); retention purge + stuck-job reconciler beat tasks; Sentry + structured logging correlation.
- **Non-goals:** no BudgetWindow table unless the Redis mechanism proves insufficient; no spend dashboard (ad-hoc GenerationAttempt queries only); does not re-implement or alter the Phase 15 demo engine or its showcase — only consumes the existing showcase as a fallback.
