...
## Phase 5B — Rights-controlled inspiration catalogue *(split from Phase 5, no renumbering)*
- **Scope:** InspirationAsset and UsageRights models; admin flows (upload → EXIF strip → WebP/thumbnail → rights record → approve); approval blocked without verified rights; public read-only catalogue endpoint (approved only).
- **Non-goals:** no frontend catalogue UI yet, no design-to-asset linking UI.
- **Commands:** `docker compose exec api python manage.py migrate`; `docker compose exec api pytest`.
- **Automated tests:** unapproved/rights-less assets never appear in the API; approval without verified rights rejected; image ingest produces WebP + thumb and strips EXIF.
...
- **Commit:** `feat(catalogue): rights-controlled inspiration catalogue`
