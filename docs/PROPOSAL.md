...
- **Inspiration influence is text-only in the MVP** — an MVP limitation, *not* a permanent decision. The provider interface must accept optional reference-image inputs later without redesign.
...
**FR5 — DesignSpec.** Fixed schema (title, editorial description, garment attributes, colour story, embellishments, styling notes, construction caveats, cultural context). Enforced at the API boundary via Anthropic structured outputs *and* re-validated in Django before storage. Never raw free-form LLM text to the UI.
...
- **DesignSession** — associated with the Django session (`session_key` reference, created lazily on first design activity), `last_seen_at`, `ip_hash` (salted, rate-limiting only). No custom token column — Django's session framework is the identity mechanism.
- **QuestionnaireVersion** — `version`, `schema` (JSONB: steps, questions, options), `is_active`. (Doubles as the taxonomy source; a separate TaxonomyOption table only if the schema JSONB proves unwieldy.)
- **InspirationAsset** — `title`, `image`, `thumbnail`, `tags` (JSONB), `alt_text`, `status` (draft/approved/retired), FK → **UsageRights** (required for approval).
- **UsageRights** — `source_name`, `source_url`, `licence_type`, `licence_text_url`, `attribution_required`, `attribution_text`, `verified_by`, `verified_at`.
...
Base: `/api/v1/`, always reached same-origin (dev rewrite / prod reverse-proxy per §6). Identity via the Django session cookie; unsafe requests carry the Django CSRF token; DRF throttle classes per scope. Schema served at `/api/schema/` (drf-spectacular).
...
**Step C — Deterministic image-prompt builder.** A pure, versioned function in `apps/generation/prompt_builder.py`:
- `PROMPT_BUILDER_VERSION` constant, recorded on every DesignVersion.
- Fixed field ordering (garment → silhouette → colours → embellishments → dupatta → setting/presentation).
- Appends fixed presentation instructions (studio-style presentation, full garment visible, modest styling defaults) and exclusion instructions (no logos, no text, no named-designer imitation — designer names are already denylisted at input, and the builder never interpolates raw free text into stylistic authority positions; free text contributes only as sanitised preference fragments in a fixed slot).
- Covered by **snapshot-style tests**: golden DesignSpec fixtures → exact expected prompt strings; any builder change forces a reviewed snapshot update and a version bump.
...
- A standalone, budget-capped experiment (`experiments/model-eval/`, plain Python scripts — not Django) run manually with real keys.
- **Prompt matrix:** representative South Asian bridalwear briefs covering at minimum: lehenga, saree, gharara, sharara, anarkali, shalwar kameez; ceremony contexts nikah, mehndi, baraat, walima; modest full-sleeve styling; heavy embroidery and minimal embroidery variants. (~24–36 prompts, hand-written to mirror what the future prompt builder would emit.)
- **Candidate models — selected at the start of Phase 2, not now.** The first experiment step is inspecting the then-current official Replicate / Black Forest Labs model catalogue and choosing: one inexpensive/fast model, one balanced production-quality model, one highest-quality model, and optionally one reference-image/editing-capable model. The exact provider model identifiers *and versions* tested are recorded in the decision record. (Illustrative, explicitly time-sensitive examples as of this writing: the FLUX Schnell / Dev / 1.1 Pro tiers — the actual set comes from the live catalogue.)
- **Scoring rubric (1–5, human-scored):** garment accuracy, cultural coherence, fabric realism, embroidery quality, dupatta styling, anatomy, prompt adherence, and **refinement consistency** (re-run a subset with one attribute changed, same seed, and score how much unrelated content drifted — this directly calibrates the honesty of the refinement UX).
- **Outputs:** an HTML/Markdown contact sheet per model, a scoring spreadsheet, and a written decision record (`docs/decisions/0001-image-model.md`) naming the chosen default model + demo/fast tier and the observed refinement-drift behaviour.
...
1. **Candidate FLUX models for Phase 2:** chosen at the start of Phase 2 from the then-current Replicate/BFL catalogue (one fast/cheap, one balanced, one highest-quality, optionally one reference-image/editing-capable); any names in this document are illustrative and time-sensitive; exact model identifiers and versions are recorded in the decision record; nothing is committed before the experiment completes.
2. Phase 2 experiment budget: on the order of USD 10–30 — confirm before running.
