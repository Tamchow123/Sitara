...
4. **Generate** — user reviews a summary, hits Generate. API enqueues an async job and returns 202; UI shows staged progress ("Interpreting your brief… Composing your concept… Rendering…") via status polling. *Delivered in Phase 12 (ADR 0013):* the idempotent Generate action, the durable job-resume `latest_job` field, and the TanStack Query progress UI with a `created_at`-derived backoff schedule and exhaustive friendly error mapping — behind the still-pending Phase 10 paid live checkpoint and `LIVE_GENERATION_ENABLED`.
5. **Concept view** — rendered image + structured description (title, garment breakdown, colour story, embellishments, styling notes, construction caveats, cultural notes) + constructability disclaimer. Actions: download image, copy description, refine. *Delivered in Phase 12 (ADR 0013):* the private result page rendering the complete curated result, prominent disclaimers, and copy/download-brief plus download-image actions, refreshing signed image URLs while open. *Refinement delivered in Phase 14 (ADR 0015).*
6. **Refinement (optional, one round)** — constrained refinement chips (colour direction, embellishment density, neckline/sleeves, dupatta style) plus a short capped note. New async job; v2 shown alongside v1, with an in-UI note that the refined render is a fresh generation and may differ in composition. *Delivered in Phase 14 (ADR 0015):* eight allowlisted change categories, a 300-character capped note (safety-scanned, never exposed in any result payload), a mandatory drift-acknowledgement gate before submission, and a side-by-side v1/v2 comparison with honest drift disclosure — behind the still-pending Phase 10 paid live checkpoint and `LIVE_GENERATION_ENABLED`.
...
**FR4 — Concept generation (async).** Pipeline: Claude Sonnet 5 → schema-constrained **DesignSpec** (structured output) → Django-side re-validation → **deterministic versioned prompt builder** → Replicate FLUX render. Job status pollable; failures surface a friendly retryable error with a stable domain error code. *The pollable job status and its frontend consumption are delivered in Phase 12 (ADR 0013):* a dedicated result endpoint and the TanStack Query progress/result UI, both provider-call-free; live provider execution stays behind the Phase 10 paid checkpoint and `LIVE_GENERATION_ENABLED`.
...
**FR6 — Refinement.** Exactly one refinement per design. Constrained option set + capped free text; Claude edits the stored DesignSpec ("change only what was asked"); the prompt builder regenerates the FLUX prompt; the image is regenerated from scratch. UI and docs state plainly that the result may substantially differ from v1. *Delivered in Phase 14 (ADR 0015):* `MAX_REFINEMENTS = 1` enforced via `DesignVersion.parent_version`/`refined_versions`; a strict `RefinementRequest` (one of eight allowlisted categories + a bounded, safety-scanned note); a separate, independently versioned refinement structured-output prompt whose candidate output is diff-checked field-by-field against a per-category DesignSpec-path allowlist (never trusting the model's own claim of what changed); the same deterministic image-prompt builder and FLUX model as initial generation, reused unchanged; seed reuse documented everywhere as a continuity aid, not a guarantee.
**FR7 — Privacy default.** Designs private to their session. No public URLs to designs, no user-design gallery.
**FR8 — Demo mode (zero-cost).** `DEMO_MODE=true` is a deterministic, production-shaped local generation branch, not a fixture lookup bolted onto the paid path: a local builder assembles a real, schema-valid DesignSpec from the validated questionnaire, the same deterministic image-prompt builder turns it into a prompt, a deterministic reviewed fixture selector picks the nearest curated concept image, and the result runs through the same durable pipeline, job/status states (queued → running_text → running_image → succeeded), private storage, result API and refinement flow as live generation — only the paid text and image provider stages are replaced. **Never calls Anthropic or Replicate — structurally guaranteed (no provider client is ever constructed in demo mode), not just configured.** *Delivered in Phase 15 (ADR 0016):* `resolve_generation_mode()` as the three-mode (`demo`/`live`/`unavailable`) public source of truth with demo taking absolute precedence over every paid flag and no live fallback; a versioned demo-asset manifest with exact garment hard filtering, weighted scoring and a stable tie-break; persisted `is_demo` identity frozen at creation and never re-derived from live settings; the reviewed production asset pack remains a separate, pending, manually budgeted checkpoint. See Phase 15 in [PHASES.md](phases/PHASES.md).
**FR9 — Live generation (separate feature).** `LIVE_GENERATION_ENABLED=true` allows real provider calls, governed by per-session and per-IP rate limits, a daily generation count limit, and a hard cost ceiling. When disabled or exhausted: 503/429 with stable error codes (§8). Demo mode and live generation are independent flags; a public portfolio deploy runs `DEMO_MODE=true, LIVE_GENERATION_ENABLED=false` by default. *Controls delivered in Phase 16 (ADR 0017):* atomic reserve-before-spend micro-USD cost ceiling on dedicated persistent Redis, ownership-first per-session/hashed-IP throttles and a global UTC daily count limit, with `503 live_generation_disabled`/`429 generation_limit_reached`/`503 live_generation_budget_exhausted`. Live generation remains disabled and provider pricing is operator-configured and unverified; the manual budgeted live checkpoint is still pending. See Phase 16 in [PHASES.md](phases/PHASES.md).
...
- BudgetWindow — *deliberately not added in Phase 16 (ADR 0017):* the live-generation cost ceiling is enforced by an atomic reserve-before-spend Redis+Lua ledger (integer micro-USD) with durable per-attempt cost on `GenerationAttempt`, revisited only if that proves insufficient.
...
Retention assumption: designs purged 30 days after creation (configurable via `DESIGN_RETENTION_DAYS`) by a periodic Celery Beat task — *delivered in Phase 16 (ADR 0017)*, deleting both rows and their storage objects.
...
| GET | `/showcase/` | Public pre-generated showcase concepts *(not implemented; superseded by the private demo journey — ADR 0016 keeps demo private with no public gallery, and ADR 0017 adds no showcase fallback for disabled/exhausted live generation)* |
...
- Prod access per §8: private bucket + short-lived signed URLs for design images (bearer-URL risk documented); public CDN for catalogue/showcase; optional backend-proxy upgrade path if strict enforcement is later required. *Delivered in Phase 12 (ADR 0013):* the original image also carries a separately signed, fixed-filename `download_url` (attachment content-disposition, same shared expiry as the existing inline URLs) via a narrow `inline|attachment` parameter on the existing signer — no new signing service, no proxy.
...
6. Demo-mode fixture set: a versioned, strict manifest of roughly 30–50 curated concepts (target ~36) spanning the garment/ceremony taxonomy and embellishment/coverage range, project-generated/owned/commissioned and reviewed once manually within budget — never downloaded or fabricated (Phase 15); demo images are matched to a locally-built DesignSpec by deterministic scoring, not served as-is per questionnaire answer.
