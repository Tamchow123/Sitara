...
ADRs currently on record: 0001 image model, 0002 application foundation, 0003 session authentication, 0004 private design ownership, 0005 versioned questionnaire schema, 0006 rights-controlled inspiration catalogue, 0007 OpenAPI generated client, 0008 questionnaire draft and wizard, 0009 structured design-spec generation, 0010 deterministic image-prompt builder, 0011 asynchronous generation pipeline, 0012 private design-image storage, 0013 generation progress and results, 0014 rights-safe inspiration metadata influence, 0015 single-round constrained refinement, 0016 deterministic demo mode, 0017 live-generation security and cost controls, 0018 questionnaire feedback and visual choice UX (Phase 16B), 0019 reference-image conditioning and the flux-2-max switch, 0020 private stylist annotation workspace, 0021 account render delivery by email, 0022 caller-named render attachments, 0023 an account is required to generate, 0024 the account concept gallery, 0025 the inspiration catalogue is retired from the product, 0026 in-store reference capture by phone handoff, 0027 the shop owns the design and the shared device, 0028 a refinement changes its own selection (amends 0015; ADR 0018 also carries a Phase 23 addendum on option descriptions).
...
- A `Design` may be refined at most once (`MAX_REFINEMENTS = 1`; a completed refinement is enforced via `DesignVersion.parent_version`/`refined_versions`). A refinement request must name exactly one allowlisted `change_type` — **seven** requestable categories since Phase 23 (ADR 0028) retired `styling_details`, which could change nothing the image prompt renders. `REFINEMENT_CHANGE_TYPES` is what a client may request; `PERSISTED_REFINEMENT_CHANGE_TYPES` (eight) is what a stored row or result payload may carry, because a historical refinement row is audit data that is read and never rewritten. The resulting DesignSpec diff is checked field-by-field against that category's exact allowlist and rejected if any changed path falls outside it or touches an immutable root — never trust a model's own claim of what it changed. Refinement is always a fresh text-to-image generation through the same deterministic `build_image_prompt` and selected image model as initial generation — never image-to-image editing, never sent the original image's bytes/URL/storage key. Seed reuse (when available) is documented everywhere as a continuity aid only, never a guarantee. The raw refinement note is untrusted, safety-scanned, bounded input; it must never be persisted into any public/result-facing payload — only the allowlisted `change_type` category may be exposed there.
- **A refinement changes its own canonical selection (Phase 23, ADR 0028).** ADR 0015 froze `source_selections` whole while its allowlist named narrative fields; prompt builder 8.0.0 then made canonical selections the skeleton and stopped rendering most of that narrative. Two correct decisions, an empty intersection, and a refinement that provably could not alter a character of the prompt. Each category may now change the one selection group it is named after, **version-dispatched** (`selection_semantics.py` — an explicit table over known versions, never a mapping framework; a version-1 spec has no `neckline_style`, so that category is refused with `refinement_category_unavailable` rather than accepted as a no-op). `REFINEMENT_IMMUTABLE_SELECTION_FIELDS` — `garment_type`, `ceremony`, `regional_style`, `custom_colours` — stays frozen and is checked explicitly, never left to the allowlist's silence. `schema_version` remains an immutable root.
- Freezing `source_selections` was the guarantee that it echoed validated answers exactly; that guarantee is **replaced, not dropped**. Every changed value is revalidated by `generation/refinement_selections.py` against the design's **pinned** questionnaire version (never the active one — a design on a RETIRED version must stay refinable), through `validate_questionnaire_answers` alone, because §12 forbids a second rule evaluator. Validating selections on their own with `require_complete=True` is exact only because every required question in every committed version is a selection field; a structural test pins that, and a future version that breaks it fails **closed**. Failures carry the failing question ids only — never the rejected value, the questionnaire contents or a raw validator message. A refined version's `source_selections` is therefore "answers that questionnaire would have accepted", NOT a record of what the user personally chose; the parent version still holds that.
- A `colour_story` refinement is sent **no reference images** (ADR 0028) — a reference photograph outweighs a text clause on colour, so "make it green" against a red reference kept losing. Every other category keeps ADR 0019's conditioning unchanged.
- Demo refinement applies the same canonical edit from candidate values computed by the same validator that judges the result. The demo pack is small, so the same asset is often reselected; that is **disclosed** (`demo_asset_unchanged`), never hidden, and demo output is never presented as a fresh provider render.
- **The demo engine is held to the same rendered-prompt standard it will be judged by.** `build_demo_refined_spec` compares its own candidate's prompt against the source's and raises `DemoRefinementInert` when they match, rather than returning a spec the service's guard would then reject. The distinction matters because this engine is *deterministic*: an inert candidate would be reproduced byte-identically on the retry, so the attempt would fail permanently. It carries the category only, never spec content and never the note, and the engine never invents a value to escape it — with no legal alternative and no narrative the builder renders, refusing is the honest answer.
- `DemoRefinementInert` is answered with **`refinement_no_change`**, matching the live path for the same fact. NOT `refinement_category_unavailable`: that code means a permanent, schema-version-level absence, and its customer copy says so — "this concept was created from an earlier version of the questionnaire, which has no such choice to change" — which would be a false statement here, because the handler is only reachable once the enqueue guard has confirmed the category does own a field. A candidate that fails to *validate or render* is a third thing again — an engine defect, not a fact about the concept — so `demo/provider.py` converts `ValidationError`/`UnsupportedDesignSpecVersion`/`ImagePromptBuildError` at the adapter boundary into `StructuredDesignProviderError`, narrowly and never as a bare `except Exception`, rather than letting them escape `provider.generate()` (whose only caller-side handler catches that one type) and surface as an unclassified internal error.
- **A demo failure must never look like unresolved spend.** `StructuredDesignProviderError` defaults to `ambiguous_acceptance=True` so an unclassified raise fails closed: the attempt terminates as `structured_submission_ambiguous`, `text_submission_in_flight` is deliberately LEFT set, and the enqueue guard then refuses every later refinement of that design — permanently, since stuck-job reconciliation only visits in-progress rows. Right for a network call that may have landed; categorically wrong for the demo engine, which sends nothing anywhere. So the demo provider raises with `ambiguous_acceptance=False` (the "provably before any request was sent" case the flag's own docstring reserves), and the Celery task boundary's catch-all passes `clear_text_marker=attempt.is_demo` so the same guarantee holds for an exception type nobody enumerated. Live behaviour is unchanged. The invariant behind both is the one `cost_accounting.cost_enabled()` already rests on: a demo attempt cannot have spent anything by any path. Never let a zero-cost local defect cost a design its one refinement.
- **The demo engine's two lookup shapes must not be confused.** `_note_keyword` takes a KEYWORD map (needle → machine value); `_note_machine_value` takes a field and searches a phrase VOCABULARY (machine value → English). Reading a hint out of a vocabulary with the former returns English, which the caller then uses as a key. That mistake shipped four times — masked because `COLOUR_PHRASES["red"] == "red"` for most entries, and a `KeyError` for the rest, from notes as ordinary as "make it maroon". Three structural tests hold it closed: every registered map's values land in a declared set; every `_*_KEYWORDS` module global appears in `NOTE_KEYWORD_MAPS`; and an `ast` scan of the module's own source requires every `_note_keyword` call site to pass a registered map. Do not add a keyword map without registering it, and do not weaken the call-site scan.
- **A refinement must actually move the render.** `_assert_the_image_prompt_actually_moved` compares the RENDERED image prompts of the source and refined specs and refuses an attempt whose prompt is unchanged — the spec-level diff is not enough, because any narrative tweak satisfies it and builder 8.x renders almost no narrative. Compared as prompts rather than requiring a canonical field to move, because builder 8.x drops `fabrics_and_texture` only *when canonical fabrics exist*, so a concept without them renders its narrative. Both prompts are built with the CURRENT builder, never against the source row's persisted `image_prompt`. An exhausted refinement fails with `RefinementNoChangeProduced` rather than persisting an identical-looking concept. `REFINEMENT_TEMPLATE_VERSION` is `5.0.0`: the model is told it MUST move a canonical selection, that a null one was left unanswered rather than forbidden, the **exact allowlist its output is graded against** (4.0.0) and the **exact values each changeable selection may be set to** (5.0.0).
- **The model is told what it is graded against (`REFINEMENT_TEMPLATE_VERSION` 4.0.0).** `refinement_allowed_paths(change_type, schema_version)` decided whether an attempt was accepted and was never transmitted; the model was asked to judge which fields were "relevant to the selected category" against a table only the server could see. A live refinement failed both attempts on it: the customer's note asked for two things at once, only one of them inside the category she picked, and the model applied the whole note. The message now carries that list as `editable_design_spec_paths`, built by **the same call with the same arguments** as the grading check — `editable_paths` is a REQUIRED keyword argument, never defaulted, because `()` would silently transmit "you may change nothing". The system prompt also states that a note may ask for more than its category covers, that only the covered part may be applied, and that ignoring the rest is correct and is not a reason to change nothing.
- **The model is told which VALUES are legal (`REFINEMENT_TEMPLATE_VERSION` 5.0.0).** 4.0.0 named the changeable FIELDS and still warned "never invent a selection value — a value it does not offer will be rejected and nothing will be saved", which instructs the model to consult a list it has never seen. `answerable_selection_alternatives` had been handing exactly that list to the DEMO engine since ADR 0028, each candidate proved by substitution through the validator that judges the result; its own docstring named the asymmetry. The live path now sends the same thing as `changeable_selection_values`, computed once per request from the design's **pinned** questionnaire — measured at 53–272 characters and about a millisecond. Declared options are NOT enough and a test refuses them: an option can be real and still illegal in context under `restrict_options` or a garment-dependent restriction.
- The single retry names WHY the previous attempt was refused, from `REFINEMENT_RETRY_NOTES` — a closed table of fixed, source-controlled strings that interpolate nothing (no field name, no rejected value, never the note). `_REJECTION_RETRY_REASONS` is total over `RefinementOutputCategory`, so a new category must choose an instruction rather than inherit one. Content-dependent failures — a failed safety scan, an invalid shape, an unsupported version, an unrenderable prompt — deliberately get the **generic** note: naming the safety check would invite the model to word its way around the denylist on the one retry available. `RETRY_REASON_UNSPECIFIED` is distinct from `None`; collapsing them would send a corrected attempt out carrying no correction.
- Phase 23 deliberately does **not** move `PROMPT_BUILDER_VERSION`: it changes what the builder is given, not how it renders. `REFINEMENT_TEMPLATE_VERSION` moved to `2.0.0`, then to `3.0.0` when the live path proved that permission alone was not enough, then to `4.0.0` when it proved that instruction alone was not enough either (the model was being graded against an allowlist nobody had shown it), then to **`5.0.0`** for the same defect one field further in — it was told which selections it could change and never what they could be changed to; `DEMO_REFINEMENT_TEMPLATE_VERSION` moved to `2.0.0` and stays there, because the demo engine's *output* contract did not change again — only its refusal to return output it knows is inert. The honest claim about this work is that **the refined prompt differs, and differs in the requested field** — not that the image visibly changes; that is an operator-run live checkpoint and no provider call has been made by this workflow at any point.
...
**Option descriptions are all-or-none per question (Phase 23, addendum to ADR 0018).** `ChoiceOptionCard` mounts its info trigger only for an option carrying a `description`, so one described option among eleven bare ones does not read as "here is help" — it reads as "this option is the unusual one". Questionnaire **v5** is a new DRAFT filling `fabrics` (12/12), `embellishment_styles` (13/13) and `embellishment_density` (3/3); v4 is byte-identical and a design pinned to it keeps its exact historical semantics. `regional_style` deliberately stays **0/9** — a one-line description of a living regional tradition is a claim about a community, not a UI affordance, and zero of nine is internally consistent. Do not "finish" it without the owner's cultural review; a build-script guard and a test both refuse. Colour swatches stay undescribed (a swatch is its own explanation), with `match_fabric` the one legitimate exception because it is a relationship rather than a colour. A description is presentation metadata: it must never reach `source_selections`, a DesignSpec, an image prompt, a refinement payload or any provider request. Generated by `fixtures/build_v5.py` — regenerate rather than hand-edit (§18).
