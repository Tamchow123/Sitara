...
- Phase 23 deliberately does **not** move `PROMPT_BUILDER_VERSION`: it changes what the builder is given, not how it renders. `REFINEMENT_TEMPLATE_VERSION` and `DEMO_REFINEMENT_TEMPLATE_VERSION` did move (both `2.0.0`). The honest claim about this work is that **the refined prompt differs, and differs in the requested field** — not that the image visibly changes; that is an operator-run live checkpoint and no provider call has been made.
