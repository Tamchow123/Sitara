...
- Phase 23 deliberately does **not** move `PROMPT_BUILDER_VERSION`: it changes what the builder is given, not how it renders. `REFINEMENT_TEMPLATE_VERSION` moved to `2.0.0`, then to `3.0.0` when the live path proved that permission alone was not enough, then to **`4.0.0`** when it proved that instruction alone was not enough either — the model was being graded against an allowlist nobody had shown it; `DEMO_REFINEMENT_TEMPLATE_VERSION` moved to `2.0.0` and stays there, because the demo engine's *output* contract did not change again — only its refusal to return output it knows is inert. The honest claim about this work is that **the refined prompt differs, and differs in the requested field** — not that the image visibly changes; that is an operator-run live checkpoint and no provider call has been made by this workflow at any point.
